...
import { MapPin, Package, UtensilsCrossed, Pencil, Check, XCircle, CheckCircle, Clock } from 'lucide-react'
...
export default function LivePage() {
  ...
  if (step === 'payment') return (
    <div className="fade-up" style={{ paddingTop: 28 }}>
      ...
      {paymentReceived === false && (
        <div style={{ background: 'rgba(251,113,133,0.08)', border: '0.5px solid rgba(251,113,133,0.25)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <p style={{ fontSize: 13, color: '#fb7185', lineHeight: 1.7, margin: 0 }}>Ask the requester for cash before handing over the item.</p>
        </div>
      )}
      {paymentReceived === true && (
        <div style={{ background: 'rgba(110,231,183,0.06)', border: '0.5px solid rgba(110,231,183,0.2)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <p style={{ fontSize: 13, color: 'var(--accent)', lineHeight: 1.7, margin: 0 }}>Great! Rs.{selected?.fee} earned. Thank you for the delivery!</p>
        </div>
      )}
  ...
  // ACTIVE DELIVERY SCREEN - Uber style
  if (step === 'active') return (
    <div className="fade-up" style={{ paddingTop: 28 }}>
      ...
      <div style={{ background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 16, padding: 20, marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 3 }}>
            <div style={{ width: 10, height: 10, borderRadius: '50%', background: 'var(--accent)', border: '2px solid var(--accent)' }} />
      ...
      <div style={{ background: 'rgba(251,191,36,0.06)', border: '0.5px solid rgba(251,191,36,0.2)', borderRadius: 12, padding: 14, marginBottom: 20 }}>
        <p style={{ fontSize: 12, color: '#fbbf24', lineHeight: 1.6, margin: 0 }}>Collect Rs.{selected?.fee} cash from the requester when you hand over the item.</p>
      </div>
  ...
  if (step === 'form') return (
    <div className="fade-up" style={{ paddingTop: 28 }}>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={() => setStep('browse')} style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '0.5px solid var(--border)', borderRadius: 12, padding: 13, color: 'var(--text-muted)', fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
          Back
        </button>
        <button onClick={acceptRequest} disabled={submitting || !carrierName || !carrierContact} style={{ flex: 2, background: carrierName && carrierContact ? 'var(--accent)' : 'rgba(255,255,255,0.1)', color: carrierName && carrierContact ? '#0a0a0f' : 'rgba(255,255,255,0.3)', border: 'none', borderRadius: 12, padding: 13, fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 14, cursor: carrierName && carrierContact ? 'pointer' : 'not-allowed' }}>
    ...
    <div className="fade-up" style={{ paddingTop: 28 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        ...
        {!loading && requests.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--accent)' }} className="pulse-live" />
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px 24px', background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 16 }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-faint)', lineHeight: 1.7, maxWidth: 240, margin: '0 auto' }}>Once someone posts a delivery request, it will show up here in real time.</p>
        </div>
      ) : (
        requests.map(req => {
            ...
            <div key={req.id} style={{ background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 16, padding: 16, marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 34, height: 34, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', background: tc.bg, color: tc.color, flexShrink: 0 }}>
                    {typeIcon(req.type)}
                  </div>
                  <div>
              ...
              {/* Description */}
              {req.description && (
                <div style={{ fontSize: 12, color: 'var(--text-muted)', padding: '8px 12px', background: 'rgba(255,255,255,0.02)', borderRadius: 8, marginBottom: 10 }}>
                  {req.description}
                </div>
              )}
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(110,231,183,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: 'var(--accent)' }}>
              ...
              {/* Accept button */}
              <button onClick={() => { setSelected(req); setStep('form') }} style={{ width: '100%', background: 'rgba(110,231,183,0.1)', border: '0.5px solid rgba(110,231,183,0.3)', borderRadius: 10, padding: 12, color: 'var(--accent)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}>
